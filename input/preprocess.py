...
df = pd.read_excel('input/ori_data/gold-standard-corpus-Edit-Position.xls')
...
with open('train2023.txt', 'w', encoding='utf-8') as f:
    for text, rel in zip(train_df['text'].tolist(), train_df['rel'].tolist()):
        f.write(str(rel)+' '+text+'\n')

with open('test2023.txt', 'w', encoding='utf-8') as g:
